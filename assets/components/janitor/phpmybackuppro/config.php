    ...
    if (array_key_exists($key,$validate)) {
        switch($validate[$key]) {
            case "int": if (!preg_match("#^[0-9]*$#",$value)) echo "<div class=\"red\">'".array_search($key,$conf)."' ".C_WRONG_TYPE."</div>\n";
                break;
            case "float": if (!preg_match("#^[0-9]*\.?[0-9]*$#",$value)) echo "<div class=\"red\">'".array_search($key,$conf)."' ".C_WRONG_TYPE."</div>\n";
                break;
            case "email": if ($value||$CONF['email_use']) {
                    foreach(explode(",",$value) as $value2) {
                        if (!preg_match("#^\ *[������a-zA-Z0-9_-]+(\.[������a-zA-Z0-9\._-]+)*@([������a-zA-Z0-9-]+\.)+([a-z]{2,4})$#",$value2)) {
...
if ($CONF['ftp_use'] || $CONF['dir_backup']) {
    ...
    } elseif(!isset($_SESSION['PMBP_VERSION'])) {
